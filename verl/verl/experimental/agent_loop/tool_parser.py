...
@ToolParser.register("custom_json")
class CustomJsonToolParser(ToolParser):
    """
    Parse user-defined JSON format (including thought field).
    Example: {"action": "get_caption", "thought": "...", "params": { ... }}
    """
    def __init__(self, tokenizer) -> None:
        super().__init__(tokenizer)
        # Match outermost JSON object, non-greedy
        self.tool_call_regex = regex.compile(r"(\{[\s\S]*?\})", regex.DOTALL)
                ...
                if char == '{':
                    ...
                    if brace_stack:
                        ...
                        if not brace_stack:
                            ...
                            json_strs.append(s[start_idx:i+1])
            return json_strs

        potential_jsons = find_json_strings(text)
        function_calls = []
        for json_str in potential_jsons:
            try:
                ...
                if isinstance(data, dict) and "action" in data:
                    name = data["action"]
                    if "params" in data:
                        arguments = json.dumps(data["params"], ensure_ascii=False)
                    else:
                        # If no 'params', treat all fields except action/thought as params (e.g. final_answer at top level)
                        params = {k: v for k, v in data.items() if k not in ["action", "thought"]}
                        arguments = json.dumps(params, ensure_ascii=False)
                    function_calls.append(FunctionCall(name=name, arguments=arguments))
                    if "thought" in data:
                        logger.info(f"[Agent Thought]: {data['thought']}")
            except json.JSONDecodeError:
                continue 
            except Exception as e:
                logger.error("Failed to parse custom tool call: %s", e)

        content = text
        for json_str in potential_jsons:
            # Remove extracted JSON string from original text
