#!/bin/bash
# CrossVid agent training: run from repo root.
# Set env vars to override paths (optional):
#   CROSSVID_DATA_FILE   - train/val parquet path
#   CROSSVID_MODEL_PATH  - actor model path
#   CROSSVID_CRITIC_PATH - critic model path
#   CROSSVID_CONFIG_DIR  - config directory (default: examples/sglang_multiturn/config)
...
# GPU and logging (override with env if needed)
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}"
# WANDB: set WANDB_API_KEY and WANDB_MODE in your environment
export WANDB_MODE="${WANDB_MODE:-online}"

DATA_FILE="${CROSSVID_DATA_FILE:-$REPO_ROOT/examples/data_preprocess/format_data/crossvid_train_mini.parquet}"
MODEL_PATH="${CROSSVID_MODEL_PATH:-Qwen/Qwen3-8B}"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.actor.optim.lr=5e-7 \
    algorithm.kl_ctrl.kl_coef=0.005 \
    trainer.project_name="${CROSSVID_PROJECT_NAME:-crossvid-debug}" \
    trainer.experiment_name="${CROSSVID_EXPERIMENT_NAME:-agent}" \
...
PID=$!
echo "Training started (PID: $PID). Data: $DATA_FILE"
wait $PID
